...
export default {
  ...
  methods: {
    ...
    async getNightContractCode(time, time2) {
      ...
      // allContract = allContract.map(item => {
      //   return item.replace(/[0-9]/g, '')+'ZL'
      // })
      ...
      // 3获取3分钟涨幅数据
      let high_frequencyRes = await postAction('high_frequency ', 
        {"codes":allContractCode.join(),"indicators":"open,high,low,close,change","starttime":time + " 21:03:00","endtime":time+" 21:03:00"}
        // {"codes":"NR2406.SHF","indicators":"open,high,low,close,volume,amount,change,sellVolume,buyVolume,openInterest,changeRatio_periodical","starttime":"2024-05-06 09:15:00","endtime":"2024-05-06 15:15:00"}
      )
