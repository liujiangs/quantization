  ...
  methods: {
    getHighJmp () {
      ...
      this.data.forEach((item, index) => {
        if (index === this.data.length - 1) return // 不写报错  因为需要index+1
        if (
        ...
        // 涨大高
        if (item.open>item.preClose * (1 + this.jmp*1) // 跳空高开jmp的幅度
        ...
        // 先留着
        // if (
        //   item.open < item.preClose*(1 - this.jmp*1)
